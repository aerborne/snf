...
<?php 
if(isset($_GET['SelectProjectUpdatePicture'])){
      $projprogimg_id = $_GET['SelectProjectUpdatePicture'];
      ...
      $select_image_info  = queryMysql("SELECT  projectprog_image.name as img_name,project_progress.name as project_name,projectprog_update.pupdate_id,projectprog_image.image_description,projectprog_image.path,projectprog_image.available,projectprog_update.percent_d FROM  projectprog_image left join projectprog_update on projectprog_image.pupdate_id = projectprog_update.pupdate_id  left join project_progress   on projectprog_update.projprog_id = project_progress.projprog_id where  projprogimg_id = '$projprogimg_id' and 
...
<?php 
  ...
  $run_all_content= queryMysql($select_all_content);
     while($row_all_content=mysqli_fetch_array($run_all_content)){
                          $section_id      = $row_all_content['section_id'];
                          $section_name    = $row_all_content['section_name'];
   ?>
...
<script>
 ...
 socket.emit('Set ProjectUpdateThumbnail',$('#projprog_update_id').html(),function(pupdate_id){});
           
    socket.on('Setted  ProjectUpdateThumbnail',function(data){
        $('#importPortfolioPictures').append('<div><a href = "index.php?SelectProjectUpdatePicture='+data.projprogimg_id+'"><img style = "width:250px;height:120px;overflow:hidden;" src="'+data.featpath+'" alt="" class="responsive-img " alt=""></a></div><br><br>') 
    });           
