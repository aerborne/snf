...
right JOIN projectprog_update ON project_progress.projprog_id = projectprog_update.projprog_id   order by pupdate_id desc limit 1");
    ...
    $data_directoryName = "images/ProjectProgUpdates/".$project_name."/".$rtpupdate_id."/".date ('D, F d Y')."/"; //Date Directory Name
    $directoryName = "images/ProjectProgUpdates/".$project_name."/".$new_pupdate_id."/".date ('D, F d Y')."/"; //Date Directory Name
?>
        ...
        //Uploading  Single and Multiple Files
          ...
          for($i=0; $i<count($_FILES['upload']['name']); $i++) {
            //Get the temp file path
              ...
              echo "<script>window.open('admin_meta_index.php?project-updates=$projprog_id','_self')</script>";
